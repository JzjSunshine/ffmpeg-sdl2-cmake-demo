#include "../include/vPlayer_sdl2.h"
...
int init_ffmpeg(FFmpeg_V_Param *p_ffmpeg_param, char *filePath)
{
    ...
    output_log(LOG_DEBUG, "", p_ffmpeg_param->pFormatCtx->nb_streams);
    printf("[LOG_DEUG] p_ffmpeg_param->pFormatCtx->nb_streams: %d\n", p_ffmpeg_param->pFormatCtx->nb_streams);
    //4. 查找解码器
    for (int i = 0; i < p_ffmpeg_param->pFormatCtx->nb_streams; i++)
    {
        ...
        if (pStream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            // 帧率
            g_frame_rate = pStream->avg_frame_rate.num / pStream->avg_frame_rate.den; // Numerator/Denominator 分子/分母
            p_ffmpeg_param->video_index = i;
            output_log(LOG_DEBUG, "for 循环: ", i);
        }
        if(pStream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO){
            p_ffmpeg_param->audio_index = i;
        }
    }
...
int sfp_refresh_thread(void *opaque)
{
    ...
    while (!g_sfp_refresh_thread_exit)
    {
        ...
        SDL_Delay(1000 / g_frame_rate);// 隔一定时间播放一次
    }
    g_sfp_refresh_thread_exit = 0;
    g_sfp_refresh_thread_pause = 0;
    SDL_Event sdl_event;
...
int vPlayer_sdl2(char *filePath)
{
    ...
    AVPacket *packet = NULL;                  // 音频包，图像包
    AVFrame *pFrame = NULL, *pFrameYUV = NULL;// 编码之后的数据
    int out_buffer_size = 0;
    unsigned char *out_buffer = 0;

    // sdl param
    SDL_Param_T *p_sdl_param = NULL;
    SDL_Event sdl_event; // 事件驱动
    ...
    {
        int temp_ret = 0;
        SDL_WaitEvent(&sdl_event);// 事件驱动，等待事件队列
        ...
        {// 键盘事件捕获
            if (sdl_event.key.keysym.sym == SDLK_SPACE)// 空格
                g_sfp_refresh_thread_pause = !g_sfp_refresh_thread_pause;
            if (sdl_event.key.keysym.sym == SDLK_q) // 退出
                g_sfp_refresh_thread_exit = 1;
        }
